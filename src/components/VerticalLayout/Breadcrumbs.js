    ...
    <nav className="flex overflow-x-auto scrollbar-none" aria-label="Breadcrumb">
      <ol className="inline-flex items-center flex-nowrap min-w-full">
        <li className="inline-flex items-center flex-shrink-0">
          <Link
            to="/"
            className="inline-flex items-center text-xs md:text-sm font-medium text-gray-700 hover:text-blue-600 whitespace-nowrap"
          >
        ...
        {breadcrumbs.map((item, index) => (
          <li key={index} className="flex items-center flex-shrink-0">
            <ChevronRightIcon className="w-3.5 h-3.5 md:w-4 md:h-4 mx-1 md:mx-2 text-gray-400 flex-shrink-0" />
            {item.path !== "#" ? (
              <Link
                to={item.path}

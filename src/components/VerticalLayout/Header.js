...
const Header = ({ toggleSidebar, isSidebarOpen }) => {
  return (
    <header className="fixed top-0 left-0 right-0 bg-[#edf3fd] dark:!bg-gray-900 shadow-sm dark:!shadow-gray-800/30 z-50 transition-all duration-200">
      <div className="h-16 max-w-[1920px] mx-auto flex items-center justify-between px-3 sm:px-4 lg:px-6">
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          >
            {isSidebarOpen ? (
              <XMarkIcon className="w-5 h-5 sm:w-6 sm:h-6 text-gray-700 dark:!text-gray-200" />
            ) : (
              <Bars3Icon className="w-5 h-5 sm:w-6 sm:h-6 text-gray-700 dark:!text-gray-200" />
            )}
          </button>
          <Link
            to="/"

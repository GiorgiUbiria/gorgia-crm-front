...
export const updateAgreementStatus = async (
  ...
  return defaultInstance.post(
    `/api/acceptance-delivery-agreements/${agreementId}/status`,
    {
      status,
      ...additionalData,
    }
...
export const getDepartmentAgreements = async () => {
  return defaultInstance.get("/api/acceptance-delivery-agreements/departments")
}

export const getUserAgreemnets = async () => {
  return defaultInstance.get("/api/acceptance-delivery-agreements/user")
}

export const downloadAgreement = async agreementId => {
  try {
    const response = await defaultInstance.get(
      `/api/acceptance-delivery-agreements/${agreementId}/download`,
      {
        responseType: "blob",
        headers: {
          Accept:
    ...
    link.setAttribute(
      "download",
      `acceptance_delivery_agreement_${agreementId}.${isDocx ? "docx" : "pdf"}`
    )

...
export const createAgreement = async data => {
  const response = await defaultInstance.post("/api/local-agreements", data)
...
export const updateAgreementStatus = async (
  ...
  return defaultInstance.post(`/api/local-agreements/${agreementId}/status`, {
    status,
    ...additionalData,
  })
}
...
export const getDepartmentAgreements = async () => {
  return defaultInstance.get("/api/local-agreements/departments")
}

export const getUserAgreemnets = async () => {
  return defaultInstance.get("/api/local-agreements/user")
}

export const downloadAgreement = async agreementId => {
  try {
    const response = await defaultInstance.get(
      `/api/local-agreements/${agreementId}/download`,
      {
        responseType: "blob",
        headers: {
          Accept:
    ...
    link.setAttribute(
      "download",
      `local_agreement_${agreementId}.${isDocx ? "docx" : "pdf"}`
    )

...
export const EditCalendarEventForm = ({ eventId, onSuccess }) => {
  const { data: eventData, isLoading: isEventLoading } = useGetCalendarEvent(
    eventId,
    {
      enabled: !!eventId,
    }
  ...
  const removeAttachmentMutation = useRemoveEventAttachment()
  const [files, setFiles] = useState([])
  const [tasks, setTasks] = useState(eventData?.tasks || [])
  const [guests, setGuests] = useState(
    eventData?.guests?.map(g => g.user_id) || []
  )
  ...
  const form = useForm({
    defaultValues: {
      ...
      reminder_before: eventData?.reminder_before?.toString() || "",
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        console.log("Form submission - Prepared data:", data)

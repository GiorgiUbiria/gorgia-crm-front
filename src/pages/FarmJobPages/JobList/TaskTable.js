...
const TaskTable = ({
  ...
  const canAccessTaskDetails = task => {
    ...
    return (
      ...
      currentUser?.department_id === 38 || // Farm department
  ...
  const handleRowClick = (e, task) => {
    ...
    if (canAccessTaskDetails(task)) {
      navigate(`/support/farm-tasks/${task.id}`)
    } else {
  ...
  const canShowAssignButton = task => {
    if (!currentUser?.department_id === 38) return false

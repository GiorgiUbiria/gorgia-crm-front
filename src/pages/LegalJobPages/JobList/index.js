...
import { AddLegalTaskForm } from "./components/add"
...
const TaskList = () => {
  document.title = "Legal Tasks List | Gorgia LLC"
  const [activeTab, setActiveTab] = React.useState("all")
  const navigate = useNavigate()

  const handleRowClick = useCallback(
    row => {
      navigate(`/support/legal-tasks/${row.original.id}`)
    },
    [navigate]
  )

  const { openModal, closeModal, isModalOpen, getModalData } = useModalStore()
  const isAddModalOpen = isModalOpen("addLegalTask")
  const isAssignModalOpen = isModalOpen("assignLegalTask")

  const { can, check } = useAuth()

  const hasAssignPermission = useMemo(
    () => can("role:admin|department:10"),
    [can]
  )

  const canViewAllTasks = useMemo(() => can("role:admin|department:10"), [can])
  ...
  const transformedLegalTasks = useMemo(() => {
    return (
      tasks?.map(task => ({
        id: task.id,
  ...
  return (
    <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
      ...
      >
        <AddLegalTaskForm onSuccess={() => closeModal("addLegalTask")} />
      </CrmDialog>

      <CrmDialog
        isOpen={isAssignModalOpen}
        onOpenChange={open => {
          if (!open) closeModal("assignLegalTask")
        }}
      ...
      >
        <AssignTaskForm
          onSuccess={() => closeModal("assignLegalTask")}
          task_id={getModalData("assignLegalTask")?.task_id}
        />
      </CrmDialog>
